...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Muddati o'tib ketgan topshiriqlar</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('index') }}">Bosh sahifa</a></li>
                        <li class="breadcrumb-item active">Muddati o'tib ketgan topshiriqlar</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
            ...
            <table id="dataTable" class="table table-bordered table-striped">
                ...
                @foreach($tasks as $task)
                    <tr>
                        ...
                        @php
                            $formattedDate = \Carbon\Carbon::createFromFormat('Y-m-d', $task->deadline)->format('d-m-Y');
                        @endphp
                        ...
                        <td>
                            @php
                                $specificDate = \Carbon\Carbon::today()->subDays($task->checkdeadline($task->id)); // Calculate the date
                                $today = \Carbon\Carbon::today();
                            @endphp

                            <span class="btn btn-danger">{{ $specificDate->format('d m Y') }}</span>

                        </td>

                        <td>
                            @foreach($task->expired_department($task->id) as $ds)
